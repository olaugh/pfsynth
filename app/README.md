...
Native macOS demo of the partial-model piano: `pf_partial` (Salamander- or Pianoteq-fitted tone on a 30-anchor A0–C8 grid, `experiments/partial-piano-wide`),
...
```
app/PfsynthDemo.app/Contents/MacOS/PfsynthDemo --export in.mid out.mp4 [start end] [--salamander] [--no-attack] [--binary-pedal] [--no-soft] [--gain dB] [--body dB] [--knock dB] [--noise dB]
```
...
with its challenges (polyphony, pedalling, dynamics, repeated notes, high register, low register) and writes `app/pieces.json`,
