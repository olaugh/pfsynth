...
struct PieceStats: Codable { let duration: Double; let notes: Int; let max_polyphony: Int; let notes_per_s: Double; let peak_notes_2s: Int; let vel_p5: Int; let vel_p95: Int; let repeated_rate: Double; let high_share: Double?; let notes_above_c7: Int?; let low_share: Double?; let notes_below_c2: Int? }
...
struct Piece: Codable, Identifiable, Hashable {
    ...
    static func fromFile(_ url: URL, duration: Double, notes: Int) -> Piece {
        Piece(path: url.path, composer: "", title: url.deletingPathExtension().lastPathComponent, tags: ["custom"], why: "Opened from disk.", excerpt: nil,
              stats: PieceStats(duration: duration, notes: notes, max_polyphony: 0, notes_per_s: duration > 0 ? Double(notes)/duration : 0, peak_notes_2s: 0, vel_p5: 0, vel_p95: 0, repeated_rate: 0, high_share: 0, notes_above_c7: 0, low_share: 0, notes_below_c2: 0),
...
enum Library {
    ...
    static let tags = ["polyphony", "pedalling", "dynamics", "repeated notes", "high register", "low register"]
}
